...
本文帮助你理解当前项目，并学习如何从一个空目录逐步创建类似程序。讲解以当前源码为准，包含最新的亮度审查功能。
...
剪辑与格式转换共用 `ProcessPage`。参数 `clipping=True` 时增加起止时间输入，避免复制两套几乎相同的界面和执行逻辑。
...
| 12 | 接入 FFmpeg 剪辑与转换 | `ProcessPage`、`transcode()` | 输出视频可播放，取消不留最终文件 |
